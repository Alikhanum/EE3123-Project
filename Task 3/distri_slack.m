...
% Info:
% RECOVER THE NETWORK INTO POWER BALANCE
...
if ~isempty(SlackSet)
    for s = 1 : length(SlackSet)
        ...
        if size(gens,1) > 1
            ...
            slackgen = gens(ind,:);
        % or report error
        else if isempty(gens) % generators on slack bus out of status
        ...
        if slackgen(PG) > 0
            % shed the power over the generation limit, those below the
            % limit can be asjusted by rundcpf()
    ...
    total_dmd = total_load(old_mpc);
    total_gen = sum(old_mpc.gen(:,2));
    if total_dmd >= total_gen
        rate = total_gen/total_dmd;
        new_mpc.bus(:, PD) = old_mpc.bus(:, PD) * rate;
    else 
        rate = total_dmd/total_gen;
        new_mpc.gen(:, 2) = old_mpc.gen(:, 2) * rate;
    end
end